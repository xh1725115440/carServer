...
class Task {
public:
	typedef void (*TaskCallback)(void*);	//任务回调函数，函数指针。定义了一个名为TaskCallback的类型别名
	// 该函数指针类型特点：返回类型void；一个名为TaskCallback的类型别名；
	// *TaskCallback表示这是一个 指向函数的指针 。
	/* 例如在事件驱动编程或线程池实现中，任务执行完成后会调用回调函数来处理结果或通知状态。*/
